 ...
 * 测试向量
 ******************************************************************************************/
#define PRINT(x)  { print(x); crc(x); checkOrder(x); }  // 一个打印函数，统计特性，统计和crc，一个检查是否有序 ;

template <typename T> //元素类型  模板函数
void   testVector ( int testSize ) { // testSize --> 输入的参数 argv[1]
   printf ( "\n  ==== Test %2d. Generate a random vector\n", testID++ );
   
   Vector<T> V;
   // 使用了辅助函数dice来产生随机数 --> 在share文件夹里面 --> rand.h
   for (int i = 0; i < testSize; i++) 
   { 
	   V.insert(dice(i + 1), dice((T)testSize * 3)); // 第一个表示插入位置，第二个表示插入的数
	   // (T)testSize --> 将testSize强制转换为T类型
...
//------------------------------------------------------------------------------------------------------------------------
   printf ( "\n  ==== Test %2d. FIND in\n", testID++ ); 
   PRINT ( V );
   TestFind<T> ( V, testSize ); // 使用了上面定义的模板函数
...
int main ( int argc, char* argv[] ) {
   ...
   // 调用函数testVector并指明了包含的模板类型，参数为 argv[i]
